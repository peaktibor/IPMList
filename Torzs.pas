...
BEGIN
  ...
  BEGIN
    ...
    {
    If Not SeekEoln(FilvarBe) then Read(FilvarBe,i);
    ...
    If Not SeekEoln(FilvarBe) then Read(FilvarBe, Field_Descriptor[i].Tipus);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe, Field_Descriptor[i].Hossz_Tipus);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Field_Descriptor[i].Hossz);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Field_Descriptor[i].H_Nev);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Field_Descriptor[i].R_Nev);
    If Not SeekEoln(FilvarBe) then begin
      Read(FilvarBe,st);
      aSL.Clear;
      SliceStrToStringList(st, aSL, ',');
      if aSL.Count >=1 then begin
        if Uppercase(Trim(aSL[0])) = 'TOHEX' then Field_Descriptor[i].Conv   := cnvToHex;
      end;
      if aSL.Count >=2 then begin
        if Uppercase(Trim(aSL[1])) = 'Y' then Field_Descriptor[i].ForceBlank := true;
      end;
      if aSL.Count >=3 then begin
        if Uppercase(Trim(aSL[2])) = 'Y' then Field_Descriptor[i].NoShow     := true;
      end;
      if aSL.Count >=4 then begin
        Field_Descriptor[i].DefLenStr := ShortString(Trim(aSL[3]));
        Field_Descriptor[i].DefLen    := StrToInt(Trim(aSL[3]));
        if length(Field_Descriptor[i].DefLenStr) < 2
          then Field_Descriptor[i].DefLenStr := '0' + Field_Descriptor[i].DefLenStr;
  ...
  While Not Eof(FilvarBe) Do
  BEGIN
//    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Rec_Descr[i].Kod);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Rec_Descr[i].MTID);
    Rec_Descr[i].Kod := Rec_Descr[i].MTID;
//    If Not SeekEoln(FilvarBe) then Read(FilvarBe, BITMAP1);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Rec_Descr[i].FUC);
//    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Rec_Descr[i].FOC);
//    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Rec_Descr[i].FOC_MEZ);
//    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Rec_Descr[i].Verzio);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe,Rec_Descr[i].Nev);

    ReadLn(FilvarBe);

    (* A K…T KARAKTEREN HEX¡BAN MEGADOTT BITMAP-et *)
    (* ¡T KELL ALAKÕTANI ASCII K”DOKK¡: *)
{
    For m := 1 to 32 Do BITMAP1[m] := UpCase(BITMAP1[m]);
    Rec_Descr[i].BitMapCh := '';
    ...
    Begin
      m := Ord(BITMAP1[j]);
      n := Ord(BITMAP1[j + 1]);
      If m in [65 .. 70] then m := m - 55 Else m := m - 48;
      If n in [65 .. 70] then n := n - 55 Else n := n - 48;
      m := (m * 16) + n;
      Rec_Descr[i].BitMapCh := Rec_Descr[i].BitMapCh + Chr(m);
      j := j + 2;
    End;
}
(*    For j := 1 To 32 Do
...
{   WriteLn(i:4, Rec_Descr[i].Kod:8,'  ',
  ...
  BEGIN
    ...
    {
    If Not SeekEoln(FilvarBe) then Read(FilvarBe,i);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe, PDS_Field_Descr[i].Tipus);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe, PDS_Field_Descr[i].Hossz_Tipus);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe, PDS_Field_Descr[i].Min_Hossz);
    If (PDS_Field_Descr[i].Hossz_Tipus = 'V') and (Not SeekEoln(FilvarBe))
      then Read(FilvarBe, PDS_Field_Descr[i].Max_Hossz);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe, PDS_Field_Descr[i].H_Nev);
    If Not SeekEoln(FilvarBe) then Read(FilvarBe, PDS_Field_Descr[i].R_Nev)
                              else PDS_Field_Descr[i].R_Nev := PDS_Field_Descr[i].H_Nev;
    }
...
{    WriteLn(i:4, Sub_Field_Descr[i].Kod:8,
               Sub_Field_Descr[i].Tipus:4,
               Sub_Field_Descr[i].Hossz:3,' ',
               Sub_Field_Descr[i].H_Nev:35,'  ',
               Sub_Field_Descr[i].R_Nev);
}
