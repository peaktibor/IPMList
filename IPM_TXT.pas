...
Procedure IPM_TO_TXT(FileNev1, FileNev2 : String);
...
Procedure MezoFormazas3(Al_Rec_Id_ : integer);
Var i, j,m,n : integer;
    hossz: integer;
    s    : AnsiString;
    PS   : AnsiString;
    ...
    Begin
      ...
      Begin
        ...
        Begin
          ...
          Writeln('Nem talßlom a k÷vetkez§ almez§t: ',PS:10);
...
Procedure MezoFormazas2(Al_Rec_Id : integer);
Var i, j : integer;
    hossz: integer;
    s    : AnsiString;
    PS   : AnsiString;
 ...
 {For i := 1 To Sub_Rec_Descr[Al_Rec_Id].Mezo_Db Do}
  ...
  Begin
   {j := Field_Find(Sub_Rec_Descr[Al_Rec_Id].Mezok[i]);}
    ...
    If j = 0 Then
    Begin
     {Writeln('Nem találom a következõ almezõt: ',Sub_Rec_Descr[Al_Rec_Id].Mezok[i]:10);}
      PS := AnsiString(Alabont[Al_Rec_Id].P_SRD.Strings[i - 1]);
      Writeln('Nem talßlom a k÷vetkez§ almez§t: ',PS:10);
...
Procedure Azonositok; (* AZONOSÍTÓK BEOLVASÁSA A REKORDBÓL *)
...
Begin
    ...
    Begin
      ...
      If k1 = -1 Then
      Begin
        WriteLn('Ismeretlen a BITMAP ',i,'. bitje által jelzett mezõ: ',j);
        Halt;
      End;
      ...
      Begin
        ...
        val(st, k, n);
        If n > 0 Then Begin
          WriteLn('Invalid field length: ' + st);
          Halt;
        End;
...
Function CopyFile(S,D:string):integer;
Var RecLen : integer;
...
Begin
  ...
  DeleteFile(D);
  OutFile := FileCreate(D);
  InFile  := FileOpen(S,fmOpenRead);
  RecLen  := FileRead(InFile,Buffer,512);
  While RecLen <> 0 Do
  Begin
    FileWrite(OutFile,Buffer,RecLen);
    RecLen  := FileRead(InFile,Buffer,512);
  End;
  FileClose(InFile);
  FileClose(OutFile);
 ...
 *)
  ...
  Rewrite (FilVarki);
  //WriteLn('Input File: ' + FileNev1);
  InFile  := FileOpen(FileNev1,fmOpenRead);
  If InFile < 0 Then
  Begin
    WriteLn('FileOpen failed : ' + FileNev1);
    Halt;
  End;
  ...
  If ReadLen <> 4 Then
  Begin
    WriteLn('Failed to read input file');
    Halt;
  End;
  ...
  If block_1014 Then
  Begin
    WriteLn('Layout: RDW with 1014 blocking');
    rc := Read_1014_Bytes(RDW_Str,4);
    If rc <> 0 Then Begin WriteLn('Failed to read input file'); Halt; End;
  ...
  End Else
  Begin
    WriteLn('Layout: Without RDW');
    If (not NativeAscii) and (RDW_Str[1] = '1') Then
    Begin
      WriteLn('It seems ASCII...');
      NativeAscii := True;
    ...
    Act_Rec_Tart.Mezo_Db := k;
{
    ...
    Writeln('ID:',EbcdicToAscii(S),'Mezo Db:',Act_Rec_Tart.Mezo_Db);
    ...
    Azonositok;  (* AZONOSÍTÓK BEOLVASÁSA A REKORDBÓL *)
    i := 1;
    ...
    If Akt_Rec = 0 Then
    Begin
      WriteLn('Nem Ismert!', MTID,'  ',BITMAP,'  ',FUC,'  ',FOC);
      Ismert := False;
      UnKnown := UnKnown + 1;
    End
    Else Begin
     ...
     { WriteLn('Ismert, neve: ',Rec_Descr[Akt_Rec].Nev); ReadLn;}
    End;
...
{     WriteLn(FejRovat);}
      ...
      Begin
        ...
        Begin
          MezoFormazas3(Al_Rec_Id);
        End Else
        Begin
          m := Pos_Header;
          MezoFormazas2(Al_Rec_Id);
          n := Pos_Header - m - 1;
          Pos_Header := m;
...
{     WriteLn('Fejrovat pozíció: ',Char_Db_B:5,'Adatsor pozíció: ',Char_Db_A:5);}
    End;
    ...
    End;
{
  ...
  WriteLn(FilvarKi);
  WriteLn(FilvarKi,'ÜZENET-TÍPUS ÖSSZESÍTÉSEK:');
  WriteLn(FilvarKi);
  j := 0;
  st := '............................................';
  For i := 1 To 80 Do
  Begin
    ...
    Begin
      WriteLn(FilvarKi,Rec_Descr[i].Nev,
                       '(',
                       Copy(Rec_Descr[i].Verzio,1,1),'.',
                       Copy(Rec_Descr[i].Verzio,2,1),
                       ')',
                       Copy(st,1,51-Length(Rec_Descr[i].Nev)),':'
  ...
  If UnKnown <> 0 Then
  begin
    WriteLn(FilvarKi,'Ismeretlen Rekord:':57,UnKnown:10);
    j := j + UnKnown;
  End;
  WriteLn(FilvarKi,'1240-es rekordok mennyisége összesen:':57,Pr1240Db:10);
  WriteLn(FilvarKi,'REKORDOK SZÁMA ÖSSZESEN:':57, j:10);
  WriteLn(FilvarKi);
